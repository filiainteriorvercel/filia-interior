...
<section class="py-20 bg-white relative overflow-hidden" id="about-section">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div class="lg:pl-8 images-container">
                @if(count($companyImages) > 0)
                    <div class="grid grid-cols-2 gap-6 image-grid">
                        @foreach(array_slice($companyImages, 0, 4) as $index => $image)
                            <div class="image-card relative group overflow-hidden rounded-2xl {{ $index % 2 == 0 ? 'mt-8' : '' }}" style="animation-delay: {{ $index * 0.2 }}s;">
                                <img src="{{ asset($image) }}" alt="Company Image" class="w-full h-64 object-cover transition-transform duration-700 group-hover:scale-110">
                ...
                @else
                    <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-12 text-center border border-gray-200 placeholder-content">
                        ...
                        <p class="text-gray-500 font-medium">Tambahkan foto perusahaan di folder public/images/company/</p>
                    </div>
                @endif
            </div>
        </div>
